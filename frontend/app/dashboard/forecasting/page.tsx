...
export default function ForecastingPage() {
  ...
  useEffect(() => { loadForecast() }, [periods, session.hasData, session.active])

  const hasData = data && (data.actual?.length > 0 || data.forecast?.length > 0)
      ...
      <div style={{ padding: 32 }}>
        ...
        {loading ? (
          ...
          <NoDataState
            ...
            description="Statistical forecasting requires historical revenue data from an active session. Upload a CSV or XML dataset on the Dashboard and click 'Start Analysis'."
          />
        ) : (
          <>
            {/* Controls */}
