...
import { getAlerts, formatCurrency, getUploadStatus } from '@/lib/api'
...
export default function AlertsPage() {
  ...
  const loadAlerts = async () => {
    setLoading(true)
    try {
      const [res, status] = await Promise.all([getAlerts(), getUploadStatus()])
      setAlerts(res)
      setSessionActive(status.has_data && status.active)
    } catch (e) {
      console.error(e)
    }
    setLoading(false)
  ...
  useEffect(() => { loadAlerts() }, [session.hasData, session.active])
  ...
  return (
    <div>
      ...
      <div style={{ padding: 32 }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 60, color: 'var(--text-muted)' }}>Scanning for anomalies...</div>
        ) : !sessionActive ? (
          <NoDataState
            title="No Active Dataset for Anomaly Alerts"
            description="Anomaly detection scans historical performance to detect revenue drops and underperforming units. Please upload a dataset on the Dashboard and click 'Start Analysis'."
          />
        ) : (
          <>
            {/* Summary */}
            ...
            <div className="glass-card animate-in animate-in-delay-2" style={{ padding: 24 }}>
              ...
              {allAlerts.length === 0 ? (
                <div style={{ textAlign: 'center', padding: 48 }}>
                  ...
                  <p style={{ color: 'var(--text-secondary)', fontSize: 15 }}>No anomalies detected in your uploaded dataset. Performance is on track!</p>
                </div>
              ) : (
